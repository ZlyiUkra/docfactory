...
# джерело: https://github.com/supabase/auth/blob/master/README.md
# отримано: 2026-09-24

Coverage Status

Auth is a user management and authentication server written in Go that powers
Supabase's features such as:
...
GoTrue codebase by Netlify, however both have diverged significantly in features and capabilities.
...
4. That's it! Visit the health check endpoint to confirm that auth is running.

## Running in production

Running an authentication server in production is not an easy feat. We
recommend using Supabase Auth which gets regular
security updates.
...
Auth uses the Semantic Versioning scheme. Here are some
...
OpenTelemetry metrics and traces to a collector.
...
1. Remap localhost to \ in your `/etc/hosts` config.
