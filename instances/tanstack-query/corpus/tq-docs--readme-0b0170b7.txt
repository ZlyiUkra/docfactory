# README
# джерело: https://github.com/TanStack/query/blob/v1.5.7/README.md
# отримано: 2026-09-26
# версія: 1.5.7, 1.5.6, 1.5.5, 1.5.4
...
A big thanks to both Draqula for inspiring a lot of React Query's original API and documentation and also Zeit's SWR and its creators for inspiring even further customizations and examples. You all rock!
...
> _These analytics are made available via the awesome Scarf package analytics library_
...
Even with just variables, mutations aren't all that special, but when used with the `onSuccess` option, the Query Cache's `refetchQueries` method and the Query Cache's `setQueryData` method, mutations become a very powerful tool.
...
- `selectedUseQueryOptions`
  - _Selected_ options of `useQuery` are also applicable here. E.g. `retry` and `retryDelay` can be used as described in the `useQuery` section. _Documentation of these options will be improved in the future._
